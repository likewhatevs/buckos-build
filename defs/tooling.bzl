...
def export_buck_config(config, output_file = "buckos_config.bzl"):
    """Generate a Buck2 configuration file.

    This creates a .bzl file that can be loaded in BUCK files.
    ...
    Returns:
        Buck2 Starlark code string
    """
    lines = [
        '"""',
        "Auto-generated BuckOs configuration.",
...
def query_package_use(package_name, iuse, config):
    ...
    """
    pkg_override = config["package_use"].get(package_name, None)
    ...
    return {
        ...
        "has_override": pkg_override is not None,
    }
...
def cmd_generate_config(profile, use_flags, output_format = "buck"):
    """Generate output for 'buckos configure' command.

    Args:
        profile: Base profile
        use_flags: Additional USE flags
        output_format: Output format (buck, json, toml, shell)
